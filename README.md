...
报告支持**定期自动抓取**固定信息源，主报告顶部展示「**最新动态与创业机会**」：

- **数据源**：Serper 搜索（中国AI创业、垂直行业、失败案例、一人公司、金融/医疗/制造/零售等行业）
- **更新频率**：每周一 + 每日一次（GitHub Actions）
- **手动触发**：仓库 → Actions → 报告自动更新 → Run workflow
- **展示位置**：主报告封面下方、一页核心结论之前

配置 `SERPER_API_KEY` 后即可启用（免费 2500 次/月）。
